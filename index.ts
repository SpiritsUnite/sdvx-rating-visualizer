const MEDALS = ["COMP", "EX", "UC", "PUC"] as const;
const GRADES = ["D",  "C",  "B",  "A",  "A+",  "AA",  "AA+",  "AAA",  "AAA+",  "S"] as const;

type Medal = typeof MEDALS[number];
type Grade = typeof GRADES[number];
type Bound = [number, number];

const MEDAL_MULT : Record<Medal, number> = {
    COMP: 1,
    EX: 1.02,
    UC: 1.05,
    PUC: 1.10,
...
const GRADE_SCORE : Record<Grade, Bound> = {
    D: [0, 65],
    C: [65, 75],
    B: [75, 87],
    A: [87, 90],
...
let bounds : Record<Medal, Record<Grade, Bound>>[] = [];

const MIN_LVL = 10;
const MAX_LVL = 20;
const VF_INTERVAL = 0.25;

const visualizer = document.querySelector(".visualizer") as HTMLDivElement;

function draw_cols(minLvl: number, maxLvl: number): Element {
...
function draw_rows(maxVf: number): Element {
...
function init_boxes(): HTMLDivElement {
    const boxes = visualizer.querySelector(".boxes") as HTMLDivElement;
    ...
    visualizer.scrollTo(
        isNaN(boxScrollLeft) ? visualizer.scrollWidth : boxScrollLeft,
        isNaN(boxScrollTop) ? 0 : boxScrollTop);
    
    if (boxes_events) return boxes;
    boxes_events = true;
    {
        let ticking = false;
...
function draw_visualizer(minLvl: number, maxLvl: number): void {
    for (let lv = minLvl; lv <= maxLvl; lv++) {
        ...
        bounds[lv] = MEDALS.reduce((bs, m) => {
            ...
            return bs;
        }, {} as Record<Medal, Record<Grade, Bound>>);
    }
